...
module SVG(A:ABSTRACT) = struct
  ...
  let src="src='https://d3js.org/d3.v5.min.js'"
  ...
  let polynew ch (f:svg) (b:elem) =
    let l = A.to_polygon b f.x f.y in
    if l = [] then () else
    (*let l = List.map (tr f) l in*)  (* For tranforming the coordinate values to pixel values*)
    Printf.fprintf ch "{";
    let l1=List.hd l in
    List.iter (fun (x,y) -> Printf.fprintf ch "{%f,%f}," x y) l;
    Printf.fprintf ch "},\n" 

  let polynew1 ch (f:svg) (b:elem) =
    let l = A.to_polygon b f.x f.y in
    if l = [] then () else
    (*let l = List.map (tr f) l in*)  (* For tranforming the coordinate values to pixel values*)
    Printf.fprintf ch "{";
  ...
  (*let poly1 (f:svg) ?(fill="none") ?(stroke="black") ?(extra="") (b:elem) =
    let channel = open_out "file.txt" in
    let l = A.to_polygon b f.x f.y in
    if l = [] then () else
    let l = List.map (tr f) l in
    Printf.fprintf channel "<polygon fill='%s' stroke='%s' points='" fill stroke;
    List.iter (fun (x,y) -> Printf.fprintf channel "%f,%f " x y) l;
  ...
  let save_image (filename:string) (f:svg) =
    ...
    (* header *)
    let channel = open_out "file.txt" in
    let channel1 = open_out "file1.txt" in
    ...
    (*coordinate axes*)
    (*Printf.fprintf ch "<line fill='white' stroke='black' stroke-width='3' x1='0' y1='0' x2='0' y2='880' />\n";*)       
    
    (* imgs *)
    Hashtbl.iter
      (fun id l -> match snd (List.hd l) with
      | Rem -> ()
      | Set (b,c) -> if c=Img then (*rect*) poly ch f ~stroke:"red" ?fill:(kind_color c) ~extra:"fill-opacity='0.5'" b 
      ) f.elems;  
    ...
    Hashtbl.iter
      (fun id l -> match snd (List.hd l) with
      | Rem -> ()
      | Set (b,c) -> if c!=Img then (*rect*) poly ch f ~stroke:"black" ?fill:(kind_color c) ~extra:"fill-opacity='0.5'" b 
      ) f.elems;

    Hashtbl.iter
      (fun id l -> match snd (List.hd l) with
      | Rem -> ()
      | Set (b,c) -> if c!=Img then (*rect*) polynew channel f b 
      ) f.elems; 

    Hashtbl.iter
      (fun id l -> match snd (List.hd l) with
      | Rem -> ()
      | Set (b,c) -> if c=Img then (*rect*) polynew1 channel1 f b 
      ) f.elems;
