...
module Soup(A:ABSTRACT) = struct
  ...
  type part = {
      ...
      mutable covers5: IdSet.t;
      mutable covers6: IdSet.t;
    }
  ...
  and elem = {
      ...
      mutable post5: IdSet.t;
      mutable post6: IdSet.t;
  ...
  let out_update_elem3 (soup:soup) (elem:elem) =
   ...
   let out_update_elem4 (soup:soup) (elem:elem) =
    ...
    let out_update_elem5 (soup:soup) (elem:elem) =
    (*if !svg_show_partition then*)
    let pid = IdSet.min_elt elem.post5 in
    let img = IdHash.find soup.imgs pid in
    (*Printf.printf "out_update";*)
    (*Printf.printf "%a\n" (print_elem soup) elem;
    SVG.save_image (Printf.sprintf "%sresult.svg" !svg_prefix) soup.svg;
    ...
    let out_update_elem6 (soup:soup) (elem:elem) =
    (*if !svg_show_partition then*)
    let pid = IdSet.min_elt elem.post6 in
    let img = IdHash.find soup.imgs pid in
    (*Printf.printf "out_update";*)
    (*Printf.printf "%a\n" (print_elem soup) elem;
    SVG.save_image (Printf.sprintf "%sresult.svg" !svg_prefix) soup.svg;
  ...
  if (r1 < r2 && r1 < r3) then r1 
   ...
   let min4 (r1, r2, r3, r4, r5) : float =
    let minr1234=min3(r1,r2,r3,r4) in
    (*let minr34=min1(r3,r4) in*)
   min1(minr1234,r5)

   let min5 (r1, r2, r3, r4, r5,r6) : float =
    let minr12345=min3(r1,r2,r3,r4,r5) in
    (*let minr34=min1(r3,r4) in*)
   min1(minr12345,r6)
  ...
  let elem_coverage (soup:soup) (elem:elem) : bool * float=
    ...
    let cntpart=0. in   (*no. of partitions out of totpart which has an abstract element from G^{\sharp}*) 
    (* accumulate over all images *)
    let covered5, totpart, cntpart = IdSet.fold cover_img elem.post5 (true,totpart,cntpart)
    in
    let coverage5=
        if covered5 then 1.
        else if cntpart=0. && totpart=0. then 0.
    ...
    let cntpart=0. in   (*no. of partitions out of totpart which has an abstract element from G^{\sharp}*) 
    (* accumulate over all images *)
    let covered6, totpart, cntpart = IdSet.fold cover_img elem.post6 (true,totpart,cntpart)
    in
    let coverage6=
        if covered6 then 1.
        else if cntpart=0. && totpart=0. then 0.
    ...
    (* normalize the coverage in [0,1] *)
      let coverage = 
      if (covered1 && covered2 && covered3 && covered4 && covered5 && covered6) then 1.
      else min5(coverage1,coverage2,coverage3,coverage4,coverage5,coverage6)
      in
    (*let coverage = 
      if covered1 then 1.
      else if cntpart=0. && totpart=0. then 0.
    ...
    in*)
    (covered1 && covered2 && covered3 && covered4 && covered5 && covered6), coverage
  ...
  let elem_img_contained3 (soup:soup) (elem:elem) : IdSet.t =
   ...
   let elem_img_contained5 (soup:soup) (elem:elem) : IdSet.t =
    IdSet.fold
      (fun iid parts ->
        let img = IdHash.find soup.imgs iid in
        IdSet.union parts img.contained
      ) elem.post5 IdSet.empty

   let elem_img_contained6 (soup:soup) (elem:elem) : IdSet.t =
    IdSet.fold
      (fun iid parts ->
        let img = IdHash.find soup.imgs iid in
        IdSet.union parts img.contained
      ) elem.post6 IdSet.empty
  ...
  let img_remove2 (soup:soup) (iid:id) =
   ...
   let img_remove3 (soup:soup) (iid:id) =
    ...
    if IdHash.mem soup.imgs iid then (
     ...
     let img_remove5 (soup:soup) (iid:id) =
    if IdHash.mem soup.imgs iid then (
      let img = IdHash.find soup.imgs iid in
      IdHash.remove soup.imgs iid;
      IdSet.iter
        (fun pid ->
          let part = IdHash.find soup.parts pid in
          part.covers5 <- IdSet.remove iid part.covers5
        ) img.contained
     )

     let img_remove6 (soup:soup) (iid:id) =
    if IdHash.mem soup.imgs iid then (
      let img = IdHash.find soup.imgs iid in
      IdHash.remove soup.imgs iid;
      IdSet.iter
        (fun pid ->
          let part = IdHash.find soup.parts pid in
          part.covers6 <- IdSet.remove iid part.covers6
        ) img.contained
     )
  ...
  let part_update_coverage2 (soup:soup) (part:part) =
   ...
   let part_update_coverage3 (soup:soup) (part:part) =
    ...
    let part_update_coverage5 (soup:soup) (part:part) =
    let to_update =
      IdSet.fold
        (fun iid acc -> IdSet.add (IdHash.find soup.imgs iid).org acc)
        part.covers5 IdSet.empty
    in
    IdSet.iter
      (fun pid -> elem_update_coverage soup (IdHash.find soup.elems pid))
      to_update

    let part_update_coverage6 (soup:soup) (part:part) =
    let to_update =
      IdSet.fold
        (fun iid acc -> IdSet.add (IdHash.find soup.imgs iid).org acc)
        part.covers6 IdSet.empty
    in
    IdSet.iter
      (fun pid -> elem_update_coverage soup (IdHash.find soup.elems pid))
      to_update
...
(*      assert(not (A.degenerate img));*)
    ...
    let elem_update5 (soup:soup) (elem:elem) (parts:IdSet.t) =
    (* remove old images *)
    IdSet.iter (img_remove5 soup) elem.post5;
    (* create new images *)
    let mk_img5 acc img =
(*      assert(not (A.degenerate img));*)
      let iid = Id.get() in
      ...
      let contained =
	    ...
	    (*Printf.printf "filter";
            ...
            print_int (IdSet.cardinal part.covers);
            read_int();*)
            if r then part.covers5 <- IdSet.add iid part.covers5;
            (*print_int (IdSet.cardinal part.covers);
            read_int();*)
            r
          ) parts
    ...
    let post4= List.hd img3 in
    let img4=soup.f post4 in
    let post5= List.hd img4 in
    let img5=soup.f post5 in 
    let post=List.fold_left mk_img5 IdSet.empty (img5) in
    ...
    elem_update_coverage soup elem;
    (* update coverage for images covered by elem *)
    part_update_coverage5 soup elem.part;
    if !verbose_log then Printf.printf "update %a\n" (print_elem soup) elem;
    out_update_elem5 soup elem


   let elem_update6 (soup:soup) (elem:elem) (parts:IdSet.t) =
    (* remove old images *)
    IdSet.iter (img_remove6 soup) elem.post6;
    (* create new images *)
    let mk_img6 acc img =
(*      assert(not (A.degenerate img));*)
      let iid = Id.get() in
      ...
      let contained =
	    ...
	    (*Printf.printf "filter";
            ...
            print_int (IdSet.cardinal part.covers);
            read_int();*)
            if r then part.covers6 <- IdSet.add iid part.covers6;
            (*print_int (IdSet.cardinal part.covers);
            read_int();*)
            r
          ) parts
    ...
    let post5= List.hd img4 in
    let img5=soup.f post5 in
    let post6= List.hd img5 in
    let img6=soup.f post6 in 
    let post=List.fold_left mk_img6 IdSet.empty (img6) in
    ...
    elem_update_coverage soup elem;
    (* update coverage for images covered by elem *)
    part_update_coverage6 soup elem.part;
    if !verbose_log then Printf.printf "update %a\n" (print_elem soup) elem;
    out_update_elem6 soup elem
  ...
  let create (f:abs -> abs list) (init:abs) (universe:abs) : soup =
    ...
    and pid = Id.get () in
    let part = { pid; space=universe; elem=None; covers=IdSet.empty; covers2=IdSet.empty; covers3=IdSet.empty; covers4=IdSet.empty; covers5=IdSet.empty; covers6=IdSet.empty;} in
    let elem = { part; abs=universe; post=IdSet.empty; post2=IdSet.empty; post3=IdSet.empty; post4=IdSet.empty; post5=IdSet.empty; post6=IdSet.empty; coverage=(-1.); coverage2=(-1.); covered=false; locked=false; } in
    part.elem <- Some elem;
    ...
    elem_update5 soup elem (IdSet.singleton pid);
    elem_update6 soup elem (IdSet.singleton pid);
    ...
    if B.equal (fst range) (snd range) then
      ...
      (*| (Nb e1, Nb e2), (Nb p1, Nb p2) ->*)  (*Earlier there were only two abstract elements after split*)
        | (e1),(p1) -> (*e1 and p1 are arrays of abstract elements*)
          ...
          let updt_img (part:part) (iid:id) =
            ...
            read_int();*)
            img.contained <- f part.pid img.contained;
            part.covers5 <- f img.iid part.covers5
          in
          IdSet.iter (updt_img partarray.(0)) partarray.(0).covers5;
          for i =1 to length-1 do
              IdSet.iter (updt_img partarray.(i)) partarray.(i).covers5;
          done;
          let updt_img (part:part) (iid:id) =
            let img = IdHash.find soup.imgs iid in
            let f =
            ...
            read_int();*)
            img.contained <- f part.pid img.contained;
            part.covers6 <- f img.iid part.covers6
          in
          IdSet.iter (updt_img partarray.(0)) partarray.(0).covers6;
          for i =1 to length-1 do
              IdSet.iter (updt_img partarray.(i)) partarray.(i).covers6;
          done;
          ...
          let parts5 = ref (elem_img_contained5 soup elem) in
          let parts6 = ref (elem_img_contained6 soup elem) in
          let parts=
          ...
          in
          let parts5=
            if IdSet.mem partarray.(0).pid !parts5 then (
              for i=1 to lengthelem-1 do 
                parts5 := IdSet.add partarray.(i).pid !parts5
              done;
	      !parts5
             )
	    else !parts5
          in
          let parts6=
            if IdSet.mem partarray.(0).pid !parts6 then (
              for i=1 to lengthelem-1 do 
                parts6 := IdSet.add partarray.(i).pid !parts6
              done;
	      !parts6
             )
	    else !parts6
	  ...
	  elem_update2 soup elem parts;
		...
		| Some e -> elem_update4 soup e parts4;
          ...
          elem_update5 soup elem parts5; 
          for i =1 to lengthelem-1 do
	     match partarray.(i).elem with
		| Some e -> elem_update5 soup e parts5;
          done;
          elem_update6 soup elem parts6; 
          for i =1 to lengthelem-1 do
	     match partarray.(i).elem with
		| Some e -> elem_update6 soup e parts6;
          done;
  ...
  let solve 
    ...
    let rec iterate step i = 
      ...
      if i > !max_iterate then 
        ...
        let useless = IdSet.is_empty elem.part.covers2 || IdSet.is_empty elem.part.covers || IdSet.is_empty elem.part.covers3 || IdSet.is_empty elem.part.covers4 || IdSet.is_empty elem.part.covers5 || IdSet.is_empty elem.part.covers6 in
        if !verbose_log then
          Printf.printf " small=%B uncovered=%B useless=%B"
            small uncovered useless;
        if (uncovered || useless) then (
