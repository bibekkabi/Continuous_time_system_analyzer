(*
  ...
  Author: Antoine Mine & Bibek Kabi
  Copyright 2015, 2018
*)
...
module Soup(A:ABSTRACT) = struct
  ...
  let elem_coverage (soup:soup) (elem:elem) : bool * float =
    ...
    let cover_img iid (covered,totpart,cntpart) =
      ...
      read_int();
      let first = ref true in
      Printf.printf "%s%a" (if !first then "" else "; ") A.output img.img;
      first := false;
      read_int();*)
      ...
      IdSet.fold
        (fun pid (covered,totpart,cntpart) ->
          ...
          then (Printf.printf "%a" A.output part.space;
                ...
                Printf.printf "%a" A.output elem.abs);*)
          covered && part.elem<>None,  (* Check for benign *)                      
          totpart,
          cntpart
        )
    ...
    IdHash.iter
      (fun pid elem ->
        Printf.printf "elem %i: abs=%a post=" elem.part.pid A.output elem.abs;
        IdSet.iter (fun iid -> let img = IdHash.find soup.imgs iid in
      Printf.printf "%s%a" (if !first then "" else "; ") A.output img.img; first :=false;) elem.post;
      ) soup.elems;

     read_int();*)

    (* accumulate over all images *)
    let covered, totpart, cntpart = IdSet.fold cover_img elem.post (true,totpart,cntpart)
    in
    (*print_float totpart;
    print_float cntpart;
    read_int();*)
...
(*      assert(not (A.degenerate img));*)
      ...
      let contained =
        IdSet.filter
          (fun pid ->
            let part = IdHash.find soup.parts pid in
            let r = A.intersect_chck img part.space in
    ...
    if B.equal (fst range) (snd range) then
      ...
      (*| (Nb e1, Nb e2), (Nb p1, Nb p2) ->*)  (*Earlier there were only two abstract elements after split*)
        | (e1),(p1) -> (*e1 and p1 are arrays of abstract elements*)
          ...
          let parts = ref (elem_img_contained soup elem) in
          let parts=
          ...
          elem.abs <- e1.(0);          
          for i=1 to lengthelem-1 do
             let elem2 = { elem with abs = e1.(i); part = partarray.(i); coverage = -1.; } in
	  ...
	  elem_update soup elem parts;
		...
		| Some e -> elem_update soup e parts;
          done;  
  ...
  let solve 
    ...
    let rec iterate step i = 
      ...
      if i > !max_iterate then 
        ...
        if !verbose_log then
          Printf.printf " small=%B uncovered=%B"
            small uncovered;
        if (uncovered) then (
