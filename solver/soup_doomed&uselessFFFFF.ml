...
module Soup(A:ABSTRACT) = struct
  ...
  let elem_coverage (soup:soup) (elem:elem) : bool * float=
    ...
    (* normalize the coverage in [0,1] *)
      let coverage = 
      if (covered1 && covered2 && covered3 && covered4 && covered5) then 1.
      else min4(coverage1,coverage2,coverage3,coverage4,coverage5)
      in
    (*let coverage = 
      if covered1 then 1.
      else if cntpart=0. && totpart=0. then 0.
    ...
    in*)
    (covered1 && covered2 && covered3 && covered4 && covered5), coverage
...
(*      assert(not (A.degenerate img));*)
  ...
  let create (f:abs -> abs list) (init:abs) (universe:abs) : soup =
    ...
    and pid = Id.get () in
    let part = { pid; space=universe; elem=None; covers=IdSet.empty; covers2=IdSet.empty; covers3=IdSet.empty; covers4=IdSet.empty; covers5=IdSet.empty;} in
    let elem = { part; abs=universe; post=IdSet.empty; post2=IdSet.empty; post3=IdSet.empty; post4=IdSet.empty; post5=IdSet.empty; coverage=(-1.); coverage2=(-1.); covered=false; locked=false; } in
    part.elem <- Some elem;
  ...
  let solve 
    ...
    let rec iterate step i = 
      ...
      if i > !max_iterate then 
        ...
        let useless = IdSet.is_empty elem.part.covers2 || IdSet.is_empty elem.part.covers || IdSet.is_empty elem.part.covers3 || IdSet.is_empty elem.part.covers4 || IdSet.is_empty elem.part.covers5 in
        if !verbose_log then
