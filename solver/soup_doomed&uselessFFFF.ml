...
module Soup(A:ABSTRACT) = struct
  ...
  let elem_coverage (soup:soup) (elem:elem) : bool * float=
    ...
    (* normalize the coverage in [0,1] *)
      let coverage = 
      if (covered1 && covered2 && covered3 && covered4) then 1.
      else min3(coverage1,coverage2,coverage3,coverage4)
      in
    (*let coverage = 
      if covered1 then 1.
      else if cntpart=0. && totpart=0. then 0.
    ...
    in*)
    (covered1 && covered2 && covered3 && covered4), coverage
  ...
  let solve 
    ...
    let rec iterate step i = 
      ...
      if i > !max_iterate then 
        ...
        let useless = IdSet.is_empty elem.part.covers2 || IdSet.is_empty elem.part.covers || IdSet.is_empty elem.part.covers3 || IdSet.is_empty elem.part.covers4 in
        if !verbose_log then
